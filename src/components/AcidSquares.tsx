...
const ctxMap = new WeakMap<Element, { renderer: InstanceType<typeof Renderer>; program: InstanceType<typeof Program>; mesh: InstanceType<typeof Mesh> }>();
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (rtA) {
        ...
        rtA.textures.forEach((tex: { texture: WebGLTexture }) => gl.deleteTexture(tex.texture));
        rtB!.textures.forEach((tex: { texture: WebGLTexture }) => gl.deleteTexture(tex.texture));
      }
      try {
